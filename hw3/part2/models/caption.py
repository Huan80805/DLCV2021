...
class Caption(nn.Module):
    ...
    def forward(self, samples, target, target_mask):
        ...
        # To get attn_weight, modify output of transformer decoder&decoder layer
        hs, attn_weight = self.transformer(self.input_proj(src), mask,
                              pos[-1], target, target_mask)
        out = self.mlp(hs.permute(1, 0, 2))
        return out, attn_weight, (src.shape[2], src.shape[3])
